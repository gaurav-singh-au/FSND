...
AUTH0_DOMAIN = 'decisions360.au.auth0.com'
ALGORITHMS = ['RS256']
...
def check_permissions(permission, payload):
    ...
    if 'permissions' not in payload:
        raise AuthError({"code": "invalid_payload",
                         "description": "No permissions in the payload"}, 401)
    if permission.lower() in [per.lower() for per in payload['permissions']]:
        return True
    raise AuthError({"code": "forbidden_access",
                     "description": "User does not have access to the \
                         resource."}, 403)
