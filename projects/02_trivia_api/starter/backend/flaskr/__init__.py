...
def create_app(test_config=None):
  ...
  def submitQuestions():
    ...
    new_ques = Question(data['question'],data['answer'],data['category'],data['difficulty'])
    # print(new_ques)
    try:
        new_ques.insert()
        return jsonify({
  ...
  def nextQuestion():
    ...
    if cat != 0:
        selection = Question.query.filter(Question.category==cat).filter(~Question.id.in_(prev_questions)).all()
    else:
